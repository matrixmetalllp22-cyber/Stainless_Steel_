...
//   "/Images/application15.png",
//   "/Images/application16.png",
//   "/Images/application17.png",
//   "/Images/application18.png",
//   // "/Images/application19.png",
//   "/Images/application20.png",
//   "/Images/application01.jpg",
//   "/Images/application02.jpg",
//   "/Images/application03.jpg",
//   "/Images/application04.jpg",
...
// DEFINING THE ARRAY (Ensure this is inside the file)
const images = [
  "/Images/application01.jpg", "/Images/application02.jpg", "/Images/application03.jpg",
  "/Images/application04.jpg", "/Images/application05.jpg", "/Images/application06.jpg",
  "/Images/application07.jpg", "/Images/application09.png", "/Images/application08.png",
  "/Images/application12.webp", "/Images/application13.webp", "/Images/application14.webp",
  "/Images/application15.png", "/Images/application16.png", "/Images/application17.png",
  "/Images/application18.png", "/Images/application20.png",
];
    ...
    <div className="min-h-screen bg-white">
      <section className="py-8 md:py-16 px-4 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10">
          <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4">Gallery</h2>
          
          {/* Controls */}
          <div className="inline-flex items-center gap-2 bg-gray-100 p-1.5 rounded-full shadow-inner">
            <Grid3x3 className="w-4 h-4 ml-2 text-gray-500" />
            {Object.keys(SPACING_OPTIONS).map((key) => (
              <button
                key={key}
                onClick={() => setSpacing(key)}
                className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase transition-all ${
                  spacing === key ? "bg-white text-blue-600 shadow-sm" : "text-gray-500 hover:text-gray-700"
                }`}
              >
                {key}
              </button>
        ...
        <Masonry
          breakpointCols={breakpointColumnsObj}
          className={`flex w-auto transition-all ${currentSpacing.container}`}
          columnClassName={currentSpacing.column}
        >
          {images.map((src, idx) => (
            <motion.div
              key={idx}
              layoutId={`img-${idx}`}
              className={`relative cursor-pointer overflow-hidden rounded-lg group ${currentSpacing.item}`}
              onClick={() => setCurrentIndex(idx)}
              whileHover={{ y: -4 }}
            >
              <img src={src} alt="" className="w-full h-auto block transform transition-transform duration-500 group-hover:scale-105" />
      ...
      <AnimatePresence>
        {currentIndex !== null && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[10000] bg-black flex flex-col items-center justify-center touch-none"
          >
            {/* Top Bar */}
            <div className="absolute top-0 w-full flex justify-between items-center p-4 z-[10001]">
              <button onClick={() => setCurrentIndex(null)} className="p-3 bg-white/10 rounded-full text-white hover:bg-white/20">
                <X size={24} />
              </button>
              <div className="text-white text-sm font-medium bg-black/50 px-4 py-1 rounded-full border border-white/20">
                {currentIndex + 1} / {images.length}
              </div>
              <button onClick={toggleFullscreen} className="p-3 bg-white/10 rounded-full text-white">
                <Maximize2 size={20} />
              </button>
            </div>

            {/* Nav Arrows (Desktop) */}
            <button onClick={handlePrev} className="hidden md:block absolute left-4 p-4 text-white/50 hover:text-white transition-colors z-[10001]">
              <ChevronLeft size={48} />
            </button>
            <button onClick={handleNext} className="hidden md:block absolute right-4 p-4 text-white/50 hover:text-white transition-colors z-[10001]">
            ...
            >
              <motion.img
                ...
                className={`max-w-full max-h-[85vh] object-contain transition-all ${isZoomed ? "cursor-zoom-out" : "cursor-zoom-in"}`}
                onClick={(e) => {
                  e.stopPropagation();
                  setIsZoomed(!isZoomed);
                }}
            ...
            {/* Mobile Footer */}
            <div className="absolute bottom-6 md:hidden text-white/40 text-xs tracking-widest">
              SWIPE TO NAVIGATE
            </div>
          </motion.div>
        )}
